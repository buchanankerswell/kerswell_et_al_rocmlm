...
## Designing RocMLM Training Data {#sec:design-training-data}
...
More importantly, MAGEMin is designed to use a limited set of predefined options for chemical systems and solution models that are tedious (but not impossible) to modify by the typical user (Riel, [personal communications](https://github.com/ComputationalThermodynamics/MAGEMin/issues/61), July 11, 2023). For example, MAGEMin does not natively include solution models for wadsleyite, ringwoodite, wuestite, perovskite, ferropericlase, and high-pressure clinopyroxene with the @holland2018 thermodynamic database---which are critical for estimating phase relations up to 28 GPa in our case. We therefore use Perple_X-generated phase relations to train RocMLMs because the analogous MAGEMin models lack key solution models.

### Bulk Compositions {#sec:bulk-compositions}

In principle, the bulk chemical composition of the upper mantle can be inverted from chemical analyses of high-pressure-high-temperature melting experiments and/or mantle-derived intrusive and extrusive igneous rocks [e.g., @allegre1984; @green1979; @ringwood1962; @jagoutz1979; @sun1982; @ringwood1991; @palme2003; @stracke2021]. [@tbl:benchmark-samples-normalized] gives four examples of hypothetical bulk mantle compositions with varying degrees of differentiation by partial melting. Two examples are commonly-referenced in the literature [Primitive Upper Mantle: PUM, and Depleted MORB Mantle: DMM, @sun1989; @workman2005], while the other two examples (Depleted Synthetic Upper MANTLE: DSUM, and Primitive Synthetic Upper Mantle: PSUM) were derived from chemical analyses of real rock samples downloaded from the online repository [Earthchem.org](https://www.earthchem.org). Below we describe our approach for deriving DSUM and FSUM, which involves two steps: 1) downloading data from Earthchem.org and normalizing the chemical analyses, and 2) defining endmember compositions using principal component analysis (PCA).

{{ benchmark-samples-normalized.md }}

First, bulk chemical analyses of real rock samples were downloaded from [Earthchem.org](https://www.earthchem.org) using the Earthchem Search Portal. The only search criterion used was: "`set sample type > igneous rocks > names from Earthchem categories > igneous/plutonic/ultramafic`", which queried 19791 samples. Samples lacking analyses for SiO$_2$, MgO, Al$_2$O$_3$, or CaO were excluded from the dataset, as well as all unknown rock types, chromitites, limburgites, wehrites, undifferentiated peridotites, and pyroxenite samples. The data were then grouped according to the remaining rock types---lherzolite, harzburgite, and dunite---and outliers (samples with any oxide compositions beyond 1.5 times the inter-quartile range of the grouped sample distribution) were removed from each group. Cr and Ni measured as trace elements (ppm) were then converted to Cr$_2$O$_3$ and NiO (wt.%) and all Fe oxides were converted to Fe$_2$O$_3$Total. Total oxides were checked against H$_2$O, CO$_2$, and LOI (if reported) to determine if chemical analyses were performed before or after ignition. Analyses with total oxides summing to $\leq$ 97 % or $\geq$ 103 % (with or without volatiles, depending on ignition) were excluded from the dataset. Finally, all analyses were normalized to a volatile-free basis before converting Fe$_2$O$_3$Total to FeOTotal. The final dataset contains 3707 chemical analyses of peridotite samples ([@fig:earthchem-harker-diagram]).

![Harker Diagrams showing the distribution of Earthchem data used for defining endmember mantle compositions. Earthchem data (colored datapoints and contour lines) indicate a trend consistent with chemical fractionation with progressive partial melting of a fertile mantle source. Black datapoints are synthetic compositions sampled randomly along a mixing array between hypothetical endmember mantle compositions PSUM (large black circle) and DSUM (large white circle). PUM [large black square, @sun1989] and DMM [large white square, @workman2005] are two commonly-referenced mantle compositions from the literature for comparison (see @tbl:benchmark-samples-normalized).](assets/figs/earthchem-harker-diagram.png){#fig:earthchem-harker-diagram}

Next, PCA was performed to reduce the dimensions of the chemical system to two principal components---PC1 and PC2---allowing endmember mantle compositions to be defined using just two values. Since PC1 and PC2 reflect the directions of maximum covariance among all oxides, PC1 and PC2 emphasize the trend from primitive-to-depleted mantle compositions indicated by the distribution of Earthchem data. Mixing arrays were then defined by computing the centroids of each rock type in PCA space (i.e., the median values for PC1 and PC2) and connecting the centroids with mixing lines. The mixing line connecting lehrzolite and harzburgite centroids was extended in the positive PC1 and PC2 directions until reaching the approximate location of the most fertile (Al$_2$O$_3$-CaO-TiO$_2$-rich) Earthchem samples. Similarly, the mixing line connecting dunite and harzburgite centroids was extended in the negative PC1 direction and positive PC2 direction until reaching the approximate location of the most refractory (MgO-rich, SiO2-poor) Earthchem samples (@fig:earthchem-mixing-arrays). Altogether, these mixing lines approximate the widest array of mantle compositions represented by the natural rock record.

![PCA diagrams showing the distribution of Earthchem data (left panel) and mixing arrays between hypothetical endmember mantle compositions PSUM and DSUM (right panel). Thick black arrows (left panel) indicate the strongest PCA loading vectors. Thick dashed line (right panel) indicates the mixing array between PSUM and DSUM, while the thick black lines indicate the upper and lower boundaries for random sampling along the mixing array. Melt fraction is calculated by applying a modal fractional melting model [after @shaw1970] to the TiO$_2$ sample concentrations.](assets/figs/earthchem-mixing-arrays.png){#fig:earthchem-mixing-arrays}

## Generating RocMLM Training Data {#sec:build-training-data}

### Gibbs Free Energy Minimization

We use the GFEM programs [MAGEMin](https://github.com/ComputationalThermodynamics/MAGEMin) and [Perple_X](https://github.com/ondrolexa/Perple_X) [@riel2022; @connolly2009] to compute RocMLM training data (rock properties) using a broad range of upper mantle PT conditions (@fig:training-dataset-design) and the two sets of bulk chemical compositions described above (@sec:bulk-compositions). MAGEMin and Perple_X compute rock properties by minimizing the total GFE of a multicomponent multiphase thermodynamic system defined by the following equation [at a fixed PT point, @gibbs1878; @spear1993]:
...
\noindent where $X_{V,1}^{\text{standardized}}$ are the standardized Ps, $X_{V,2}^{\text{standardized}}$ are the standardized Ts, $y_{V}^{\text{standardized}}$ are the standardized rock property (density in this case), $\mu$ is the mean and $\sigma$ is the standard deviation of the appropriate array. This so-called "z-score normalization" is a necessary step before RocMLM training because the difference in magnitude of the feature values (1-28 GPa vs. 773-2273 K) results in poor performance for RocMLMs that use distance-metrics for fitting.
