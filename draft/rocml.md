...
The preprocessed training data were then fit with eight different non-linear ML models (@tbl:rocml-descriptions). Each model used standard parameters from the scikit-learn python library [@scikit2011] except for K Nearest, which was fit using a distance-weighting scheme instead of uniform weighting of the k-nearest data points. For Neural Networks, dense (fully-connected) layers were used with constant layer sizes equal to the total number of training data $V$ divided by 100 and rounded down to the nearest integer. The reader is referred to the scikit-learn [documentation](https://scikit-learn.org/stable/supervised_learning.html#supervised-learning) on regression models for more specifics.

{{ mlms.md }}
: Advantages and disadvantages of various non-linear ML models. {#tbl:rocml-descriptions}
...
Model performance can be assessed qualitatively by comparing the density surfaces computed by MAGEMin and Perple_X with the density surfaces inferred by RocMLs (@fig:all-surf-comparison).
...
{{ benchmark-rocmls-performance.md }}
: RocML training performance measured by 30-fold cross-validation. {#tbl:regression-info}

![caption](assets/figs/benchmark-rocmls-performance.png)
