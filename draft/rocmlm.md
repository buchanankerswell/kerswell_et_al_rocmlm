...
\begin{equation}
	\frac{C_{\text{TiO}_2}^s}{C_{\text{TiO}_2}^0} = R = (1 - F)^{\frac{1}{k_d} - 1} \label{eq:shaw-melting}
\end{equation}

\noindent where $R$ is the ratio of the TiO$_2$ concentration of the sample to the initial PSUM source (@tbl:benchmark-samples), $F$ is the melt fraction, and $k_d$ = 0.05 is the bulk distribution coefficient for TiO$_2$ in peridotite [after @brown2016]. Depletion is calculated by rearranging @eq:shaw-melting for $F$ and subtracting it from 1:

\begin{equation}
	D = 1 - F = R^{\frac{1}{(\frac{1}{k_d}) - 1}} \label{eq:melt-fraction}
\end{equation}

Training RocMLMs on $D$ instead of nine oxide components is ideal for two reasons: 1) it greatly increases computational efficiency and 2) unlike oxide components, $F$ is routinely implemented in numerical geodynamic simulations [e.g., @cerpa2019; @gerya2003; @sizova2010; @li2019; @yang2020; @kelley2010]. Although we chose $D$ for RocMLM training to avoid confusion between $F$ used for RocMLM training (a model input) and $F$ predicted by RocMLM inference (a model output), $D$ and $F$ are otherwise interchangeable. Thus, @eq:shaw-melting offers a generalized solution for coupling RocMLMs to various geodynamic codes. Three implicit assumptions in @eq:shaw-melting are important to note, however, because they impact $F$, and thus ultimately impact RocMLM predictions: 1) melt is instantaneously removed from the source region, 2) $k_d$ is constant, and 3) minerals melt in the same proportions that they exist in the source rock. While these assumptions strongly depend on the local melting conditions in the mantle and the progression of melting reactions [@langmuir1992; @niu1997b; @brown2016], @eq:melt-fraction predicts that DMM is produced by 2.2 % melt extraction from PUM (@tbl:benchmark-samples), which is consistent with the degree of melt extraction estimated by others [2-3 % melt removal from PUM, @workman2005]. Considering that melting is normally implemented in geodynamic codes by approximating first-order isobaric melting reaction curves [e.g., @ito1967; @schmidt1998], we consider the accuracy of @eq:melt-fraction adequate for RocMLM training.
