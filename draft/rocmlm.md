...
RocMLM accuracy is both critical for geological applications (e.g., simulating mantle convection) and dependent on the regression algorithm used for RocMLM training. We therefore define RocMLM accuracy in two ways by evaluating the correlation (R$^2$) and residuals (RMSE) between RocMLM predictions and 1) the training dataset (internal accuracy), and 2) reference Earth models derived from seismological data (external accuracy). Internal accuracy quantifies how well training data were mapped to RocMLMs by regression, while external accuracy quantifies how well RocMLM predictions compare to the inferred physical properties of Earth's upper mantle. More precisely, external accuracy is evaluated by extracting a depth profile of RocMLM predictions along an average continental geotherm and comparing the depth profile to two commonly-cited reference Earth Models: the Preliminary Reference Earth Model [PREM, @dziewonski1981], and 2) STW105 [a.k.a. REF, @kustowski2008]. The reported external accuracy is therefore dependent on the geophysical inversion methodology and our choice of geotherm, which can vary widely, but nevertheless provide some indication of RocMLM accuracy. Two examples of RocMLMs are given below that demonstrate the relative internal and external accuracies of different regression algorithms (all regression algorithms are presented in @sec:training-data-mappings).
