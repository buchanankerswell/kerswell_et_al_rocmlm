...
RocMLM accuracy (in terms of R$^2$ and RMSE) was evaluated by: 1) testing RocMLMs on separate training data to determine the generalizability of RocMLMs to unknown mantle conditions (validation accuracy), and 2) comparing RocMLMs with reference Earth Models [PREM and STW105, @dziewonski1981; @kustowski2008] to test RocMLM predictions against inversions of seismological data (external accuracy). RocMLM performance, on the other hand, was evaluated by: 1) measuring single-point prediction times (execution speed), and 2) scaling execution speed by RocMLM filesize to account for information compression (efficiency).
...
Since accuracy is measured relative to the rock properties generated by GFEM programs, GFEM programs and LUTs have perfect accuracy by definition. With respect to RocMLMs, validation accuracies are observed to be roughly constant for regression algorithms that tend to overfit the training data (DT and KN, @tbl:rocmlm-performance), while NNs show a positive correlation between accuracy and dataset size (@fig:rocmlm-tradeoffs). In addition to improving accuracy with increasing amounts of training examples, NN accuracy increases with the number of hidden-layers (@fig:rocmlm-tradeoffs) because deeper networks are more capable of fitting sharp gradients in the training data (see @sec:training-data-mappings for examples of NN1, NN2, and NN3 RocMLMs).

# Discussion

## Geophysical and Thermodynamic Estimates of Elastic Properties {#sec:rocmlm-elastic-properties}

External accuracy is dependent on the geotherm and bulk mantle composition used to construct RocMLM depth profiles, and the choice of reference Earth model used as a basis for comparison. External accuracy can vary widely, but nevertheless provide an indication of how RocMLM predictions compare to inversions of seismological data.

## RocMLM Performance Tradeoffs {#sec:rocmlms-tradeoffs}

We have identified tradeoffs among prediction times, compression, and accuracy (internal and external) that allow ranking of RocMLMs in terms of overall performance.

Together with external accuracy, prediction times and efficiency are used to determine the most practical model(s) for coupling with numerical geodynamic codes.

RocMLM accuracy is both critical for geological applications and dependent on the regression algorithm used for RocMLM training. Besides internal and external accuracies, RocMLM efficiency is the most crucial metric for comparing RocMLMs with incumbent GFEM programs. 
