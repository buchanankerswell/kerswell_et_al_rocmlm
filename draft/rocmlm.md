...
RocMLM accuracy is both critical for geological applications and dependent on the regression algorithm used for RocMLM training. We therefore evaluate RocMLM accuracy (R$^2$ and RMSE) in two ways: by 1) validating pre-trained RocMLMs on a separate set of Perple_X pseudosections not seen during training, and 2) comparing RocMLMs depth profiles with two commonly-cited reference Earth Models: the Preliminary Reference Earth Model [PREM, @dziewonski1981], and STW105 [a.k.a. REF, @kustowski2008]. The former definition quantifies how well RocMLMs generalize to unknown mantle conditions (internal accuracy), while the latter quantifies how RocMLMs compare to inversions of seismological data (external accuracy). In this case, internal accuracy is a function of the regression algorithm and hyperparameters used to train RocMLMs (@tbl:rocmlm-config) and propagates to external accuracy. External accuracy is additionally influenced by the reference Earth model, geotherm, and bulk mantle composition used for RocMLM depth profiles. External accuracy can therefore vary widely, but nevertheless provides a general indication of how RocMLMs will perform when predicting rock properties in Earth's upper mantle.

Besides internal and external accuracies, RocMLM efficiency is the most crucial metric for comparing RocMLMs with incumbent GFEM programs. Like accuracy, we evaluate RocMLM efficiency in two ways: by 1) measuring single-point prediction times (in milliseconds, ms), and 2) scaling single-point prediction times by RocMLM filesize (in Megabytes, Mb). The former definition quantifies execution speed, while the latter definition also accounts for the degree of information compression. We have identified tradeoffs among prediction times, compression, and accuracy (internal and external) that allow ranking of RocMLMs in terms of overall performance, which are discussed in @sec:rocmlms-tradeoffs.

# Results {#sec:results}

## RocMLM Accuracy {#sec:rocmlms-accuracy}

The following examples of Decision Tree (DT, @fig:image12-PUM-DT) and single-layer Neural Network (NN1, @fig:image12-PUM-NN1) RocMLMs demonstrate how different regression algorithms ultimately influence the accuracy of RocMLM predictions (see @sec:training-data-mappings for all regression algorithms).

DT predictions are indistinguishable from Perple_X, with low error rates (RMSE density: 0.003 g/cm$^3$, Vp: 0.019 km/s, Vs: 0.033 km/s) that indicate a nearly-perfect mapping of the training dataset by the DT algorithm (i.e., high internal accuracy, @fig:image12-PUM-DT). The few non-zero residuals between DT and Perple_X predictions tend to concentrate at phase transitions, especially the onset of partial melting at HT-LP conditions ([@fig:image12-PUM-DT]g–i). In contrast, NN1 predictions are notably more diffuse than Perple_X (@fig:image12-PUM-NN1), with lower internal accuracy (RMSE density: 0.042 g/cm$^3$, Vp: 0.132 km/s, Vs: 0.266 km/s) that indicate an inability to resolve sharp gradients in physical properties ([@fig:image12-PUM-NN1]g–i).

Comparing Perple_X and RocMLM depth profiles with PREM and STW105 reveals minor residuals with respect to density (i.e., high external accuracy), but poor correlations, high error rates, and a systematic shift towards lower seismic velocities with respect to elastic properties (i.e., low external accuracy). Irrespective of this discrepancy, the influence of different regression algorithms on external accuracy can be evaluated by directly comparing DT and NN1 profiles. For example, the smooth NN1 profiles do not resolve the discontinuities indicated by PREM and STW105 ([@fig:image12-PUM-NN1]j–l), and even exhibit inverted Vs gradients at some depths ([@fig:image12-PUM-NN1]l). Meanwhile, the DT profiles do resolve discontinuities, especially at the $\text{olivine} \xrightarrow{\text{410 km}} \text{ringwoodite}$ and $\text{ringwoodite} \xrightarrow{\text{660 km}} \text{bridgmanite} + \text{ferropericlase}$ transitions. We note, however, that deeper NN architectures with three hidden-layers (NN3) also resolve discontinuities with roughly equivalent internal and external accuracies as DT and KN algorithms (@tbl:rocmlm-performance).

![PT diagrams showing density (left column, g/cm$^3$), Vp (middle column, km/s), and Vs (right column, km/s) predictions from a Perple_X model with a PUM bulk composition (a–c), a Decision Tree RocMLM (d–f), and differences between Perple_X and DT (g–i). Depth profiles (j–l) compare Perple_X and DT predictions extracted along an average continental geotherm (white line in PT diagrams) with reference Earth models PREM [solid black line, @dziewonski1981] and STW105 [dotted black line, @kustowski2008].](assets/figs/image12-PUM-DT.png){#fig:image12-PUM-DT}

![PT diagrams showing density (left column, g/cm$^3$), Vp (middle column, km/s), and Vs (right column, km/s) predictions from a Perple_X model with a PUM bulk composition (a–c), a single-layer Neural Network RocMLM (d–f), and differences between Perple_X and NN1 (g–i). Depth profiles (j–l) compare Perple_X and NN1 predictions extracted along an average continental geotherm (white line in PT diagrams) with reference Earth models PREM [solid black line, @dziewonski1981] and STW105 [dotted black line, @kustowski2008].](assets/figs/image12-PUM-NN1.png){#fig:image12-PUM-NN1}

## RocMLM Efficiency {#sec:rocmlm-performance-and-efficiency}

![Computational efficiency of various programs in terms prediction time (a) and prediction time scaled by filesize (b). Some programs (LUT, DT, KN) become cumbersome and/or slow and/or lose accuracy (RMSE) as the amount of training examples increases, while other progrms (NN1 and NN3) maintain high-speed, good compression, and increase accuracy. All computations were made on a Macbook Pro (2022; M2 chip) with macOS 13.4 and Python 3.11.4.](assets/figs/rocmlm-tradeoffs.png){#fig:rocmlm-tradeoffs}
...
![PT diagrams showing density (left column, g/cm$^3$), Vp (middle column, km/s), and Vs (right column, km/s) predictions from a Perple_X model with a PUM bulk composition (a–c), a Decision Tree RocMLM (d–f), and differences between Perple_X and DT (g–i). Depth profiles (j–l) compare Perple_X and DT predictions extracted along an average continental geotherm (white line in PT diagrams) with reference Earth models PREM [solid black line, @dziewonski1981] and STW105 [dotted black line, @kustowski2008].](assets/figs/image12-PUM-DT.png)

![PT diagrams showing density (left column, g/cm$^3$), Vp (middle column, km/s), and Vs (right column, km/s) predictions from a Perple_X model with a PUM bulk composition (a–c), a K-Neighbors RocMLM (d–f), and differences between Perple_X and KN (g–i). Depth profiles (j–l) compare Perple_X and KN predictions extracted along an average continental geotherm (white line in PT diagrams) with reference Earth models PREM [solid black line, @dziewonski1981] and STW105 [dotted black line, @kustowski2008].](assets/figs/image12-PUM-KN.png){#fig:image12-PUM-KN}

![PT diagrams showing density (left column, g/cm$^3$), Vp (middle column, km/s), and Vs (right column, km/s) predictions from a Perple_X model with a PUM bulk composition (a–c), a single-layer Neural Network RocMLM (d–f), and differences between Perple_X and NN1 (g–i). Depth profiles (j–l) compare Perple_X and NN1 predictions extracted along an average continental geotherm (white line in PT diagrams) with reference Earth models PREM [solid black line, @dziewonski1981] and STW105 [dotted black line, @kustowski2008].](assets/figs/image12-PUM-NN1.png)

![PT diagrams showing density (left column, g/cm$^3$), Vp (middle column, km/s), and Vs (right column, km/s) predictions from a Perple_X model with a PUM bulk composition (a–c), a two-layer Neural Network RocMLM (d–f), and differences between Perple_X and NN2 (g–i). Depth profiles (j–l) compare Perple_X and NN2 predictions extracted along an average continental geotherm (white line in PT diagrams) with reference Earth models PREM [solid black line, @dziewonski1981] and STW105 [dotted black line, @kustowski2008].](assets/figs/image12-PUM-NN2.png){#fig:image12-PUM-NN2}

![PT diagrams showing density (left column, g/cm$^3$), Vp (middle column, km/s), and Vs (right column, km/s) predictions from a Perple_X model with a PUM bulk composition (a–c), a three-layer Neural Network RocMLM (d–f), and differences between Perple_X and NN3 (g–i). Depth profiles (j–l) compare Perple_X and NN3 predictions extracted along an average continental geotherm (white line in PT diagrams) with reference Earth models PREM [solid black line, @dziewonski1981] and STW105 [dotted black line, @kustowski2008].](assets/figs/image12-PUM-NN3.png){#fig:image12-PUM-NN3}
